...
const Announcements = () => {
  ...
  return (
    <div
      className={`min-h-screen ${darkMode ? "bg-dark" : "bg-gray-50"} mb-12`}
    >
      {/* Section Title */}
      <SectionTitle
      ...
      <p className="text-accent text-center text-lg mb-6 -mt-10">
        Total Announcements:{" "}
        <span className="font-bold text-secondary">{data.length}</span>
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data.map((announcement) => (
          <div
            ...
            } shadow-lg rounded-lg p-6 border-l-4 border-primary hover:shadow-xl transition-shadow`}
          >
            <div className="flex items-center gap-3 mb-4">
              <MdAnnouncement className="text-primary text-3xl" />
              <h2
                className={`text-xl font-bold ${
                  darkMode ? "text-primary" : "text-gray-700"
                }`}
              >
                {announcement.title}
              </h2>
